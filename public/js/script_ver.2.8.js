...
/*
//MTdev
function keyDownHandler(obj,_event)
{
    if (obj.name == "contact1" || obj.name == "contact2")
    {
        if (obj.value == '+7' || parseInt(_event.keyCode) < 96 || parseInt(_event.keyCode) > 105) {
            obj.value = "+7 ";
        };
    }
}
...
{  
    ...
    if (obj.name == "addParentContact")
    {
        ...
        ajax('/skd/getStudentsLogs', function(data){document.body.querySelector('.addParentContact').querySelector('.results').innerHTML =  data;},params);
...
function loadInvData(){
    ...
    if (seachType=='invNumber') {
        document.getElementById('cancelFas').classList.add('hide');
